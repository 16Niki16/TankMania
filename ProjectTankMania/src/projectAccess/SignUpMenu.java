...
public class SignUpMenu extends JFrame {
	...
	public SignUpMenu() { //JFrame constructor
		...
		Color lightBlue= new Color(51,153,255,255);
		panel.setBackground(lightBlue);
		panel.setLayout(null);
		
		JLabel SignUpMenu = new JLabel("SIGN UP MENU");
		SignUpMenu.setFont(new Font("Tahoma", Font.PLAIN, 20));
		SignUpMenu.setBounds(111,4, 150, 20);
		panel.add(SignUpMenu);
					...
					try {
						...
						if(count == 0) {
							if(Pattern.matches("\\d{10}",textTelNumber.getText())&& Pattern.matches("[a-zA-Z0-9]+[@][a-zA-Z0-9.]+", textEmail.getText())){
								...
								String query1 = "INSERT INTO userdata(RealName, RealFamilyName, Username, Password, Email, TelephoneNumber, Gender, History)" 
										+ "VALUES('" + textName.getText() + "','" + textFamilyName.getText() + "','" + textUsername.getText() 
										+ "','" + textPassword.getText() + "','" + textEmail.getText() + "','" + textTelNumber.getText() +  "','" + n + "','"+ 0 + "')";
								//sqlQuery.updateSQL_Query(query1);
								sqlQuery.updateQuery(query1);
								dispose();
							}else{
